...
class HumanPlayMode:
    ...
    def __init__(self, time_limit_minutes=10, window_size=None):
        ...
        self.persistence_buffer = None
        self.persistence_decay = 0.98  # higher persistence to further reduce flicker
        ...
        # Rendering and simulation tuning
        self.logic_steps_per_frame = 4  # Number of env steps per displayed frame
    ...
    def _get_current_lives(self, info=None):
        # Prefer lives from info if provided by the environment
        if info is not None and isinstance(info, dict):
            lives_from_info = info.get("lives")
            if lives_from_info is not None:
                return int(lives_from_info)
        # Fallback to ALE interface
        try:
    ...
    def run(self):
        ...
        self.prev_lives = self._get_current_lives(info)
        if self.prev_lives is not None:
            print(f"Initial lives: {self.prev_lives}")
        ...
        while running:
            ...
            # Take multiple logic steps per displayed frame to restore normal speed
            terminated = False
            truncated = False
            composite = None  # fresh composite each displayed frame (no trails)
            for _ in range(self.logic_steps_per_frame):
                ...
                # Detect life loss events (non-terminal as well)
                current_lives = self._get_current_lives(info)
                if self.prev_lives is not None and current_lives is not None and current_lives < self.prev_lives:
                    print(f"Life lost at step {self.step_count}: lives now {current_lives}")
                self.prev_lives = current_lives
                
                # Build composite of substep frames (max over substeps) to remove flicker
                frame_np = self.env.render()
                if composite is None:
                    composite = frame_np.astype(np.uint8)
                else:
                    composite = np.maximum(composite, frame_np)
                
                if terminated or truncated:
                    break
            
            # Convert composite to pygame surface and scale once
            display_np = composite if composite is not None else self.env.render()
            surface = pygame.surfarray.make_surface(np.transpose(display_np, (1, 0, 2)))
            scaled_surface = pygame.transform.scale(surface, self.window_size)
            ...
            if terminated or truncated:
                ...
                obs, info = self.env.reset()
                # Reset lives tracking after reset
                self.prev_lives = self._get_current_lives(info)
